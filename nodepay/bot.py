...
async def load_proxies():
    """
    Load proxies from proxylist.txt file.
    """
    try:
...
async def call_api(url, data, account_info, proxy):
    """
    Make an API call with the specified proxy.
    """
    headers = {
...
async def render_profile_info(account_info):
    """
    Process each proxy for the account to render profile info.
    """
    try:
...
async def start_ping(account_info):
    """
    Start the ping process for the account.
    """
    try:
...
def process_account(token, proxies):
    ...
    account_info.proxies = proxies  # Assign proxies to the account
    asyncio.run(render_profile_info(account_info))


async def main():
    """
    Main function to orchestrate loading tokens and proxies and processing accounts.
    """
